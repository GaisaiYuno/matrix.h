#include "poly.h"
using namespace std;
int main(){
    cout<<EquivInf(upoly("x^2")-variableLimit(Cos("x^2"),"0","x^2"));
    // cout<<EquivInf(variableLimit(shift(-1,Sin("x")),"x^3","x^2"));
    // cout<<Deriv(Deriv(Exp("-x^2")))<<endl;
    // cout<<Integral(Pow(upoly("1+x^2"),upoly("-1")))<<endl;
    // cout<<to_latex(poly_ele("-1/2a^1/2x^4y"))<<endl;
    // cout<<EquivInf(Exp("-1/2x^2")-Cos("x"))<<endl;
    // cout<<EquivInf(Ln("1+x+x^2")+Ln("1-x+x^2"))<<endl;
    // cout<<Limit(upoly("x")*(Exp("x")+Exp("-x")-upoly("2")),upoly("x")-Sin("x"))<<endl;
    // cout<<Limit(Exp(shift(-1,Ln("1+x"))-upoly("1"))-upoly("1"),"x")<<endl;
    // cout<<Limit(2*Sin("x")*Ln("1+x")-2*upoly("x")*Arctan("x")+upoly("x^3"),upoly("x^4"))<<endl;
    // cout<<Limit(Sin(Tan("x"))-Tan(Sin("x")),upoly("x^3"))<<endl;
    // cout<<Limit(Sin(Exp("x")-upoly("1"))-(Exp(Sin("x"))-upoly("1")),upoly("x^4"))<<endl;
    // cout<<EquivInf(upoly("x")*Integral(Sin(Sin("x")*Sin("x"))*Deriv(Sin("x"))))<<endl;
    // cout<<EquivInf(upoly("x")-Sin("x")+Integral(upoly("x^2")*Exp("x^2")))<<endl;
    // cout<<Limit(Exp("1/2x")-Pow("1-x",frac(-1,2)),upoly("x")*Tan("3x"))<<endl;
    // cout<<EquivInf(Integral(Sin("x^2")))<<endl;
    // cout<<Integral(shift(-2,upoly(1)-Ln("x")))<<endl;
    // cout<<Integral(upoly("x^2009")*Exp("-1/2x^2"))<<endl;
}